(() => {
  ...
  function finalizeGateDiagnostics(analyzedRows){
    ...
    for(const row of validRows){const k=classifyEntryPrimaryGate(row);g[k]=(g[k]||0)+1;}
    state.gateDiagnostics=g;renderGateDiagnostics();
    log(`Gate Diagnostics: Fast ${g.fastEvents} | Liquidity ${g.liquidityDrop} | OI ${g.oiDrop} | Data ${g.dataDrop} | Operational ${g.operationalDrop} | Entry ${g.entryAnalyzed} | EntryError ${g.entryError} | 1hBlock ${g.oneHourBlock} | 1hTransition(info) ${g.oneHourTransition} | Fresh/Elder ${g.freshnessElder} | 15m ${g.setup15m} | 5m ${g.confirm5m} | Trigger/Zone ${g.triggerZone} | R/R ${g.rr} | Soft ${g.softCaution} | SELECT ${g.select}`);
  }
...
function logEntryDecisionDiagnostics(rows){
  ...
  window.CryptoFlowScanner=window.CryptoFlowScanner||{version:'V14.8',modules:{}};
  window.CryptoFlowScanner.version='V14.8';
