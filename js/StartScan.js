...
function finalizeEntryDecision(row,chart,finalSpread=NaN){
  ...
  if(chart.step10?.status==='READY'){
    if(!loc||!Number.isFinite(loc.structuralAsymmetry))coreCautions.push('structural R/R unavailable');
...
function logEntryDecisionDiagnostics(rows){
  ...
  window.CryptoFlowScanner=window.CryptoFlowScanner||{version:'V14.5',modules:{}};
  window.CryptoFlowScanner.version='V14.5';
