(() => {
  ...
  const state={running:false,controller:null,startedAt:0,requestCount:0,errors:[],results:[],activity:[],context:null,settings:null,thresholds:null,snapshotElapsedMs:null,fundingInfoLoaded:false,activityCache:null,eventWatch:new Map(),priceLearning:[],gateDiagnostics:null};
  ...
  function summarizeBaseDropDiagnostics(expectedDropped=NaN){
    let liquidityDrop=0,oiDrop=0,dataDrop=0;
    for(const [reason,count] of baseDropDiagnostics){
      const r=String(reason||'').toLowerCase(),n=Number(count)||0;
      if(r.includes('spread')||r.includes('depth'))liquidityDrop+=n;
      else if(r.includes('severe oi'))oiDrop+=n;
      else dataDrop+=n;
    }
    const recorded=liquidityDrop+oiDrop+dataDrop;
    if(Number.isFinite(expectedDropped)&&expectedDropped>recorded)dataDrop+=expectedDropped-recorded;
    return {liquidityDrop,oiDrop,dataDrop};
  }
  function setDiagText(id,value){const el=$(id);if(el)el.textContent=String(value??0);}
  function renderGateDiagnostics(){
    const g=state.gateDiagnostics||emptyGateDiagnostics();
    const map={gateFast:g.fastEvents,gateLiquidity:g.liquidityDrop,gateOi:g.oiDrop,gateData:g.dataDrop,gateOperational:g.operationalDrop,gateAnalyzed:g.entryAnalyzed,gateEntryError:g.entryError,gate1hBlock:g.oneHourBlock,gate1hTransition:g.oneHourTransition,gateFreshElder:g.freshnessElder,gate15m:g.setup15m,gate5m:g.confirm5m,gateTrigger:g.triggerZone,gateRR:g.rr,gateSoft:g.softCaution,gateSelect:g.select};
    for(const [id,v] of Object.entries(map))setDiagText(id,v);
    const d=g.confirm5mDetail||{};
    const dmap={gate5mTiming:d.timing,gate5mScore0:d.score0,gate5mScore1:d.score1,gate5mScore2:d.score2,gate5mScore3:d.score3,gate5mWeakVol:d.weakClosedVolume,gate5mFailEma:d.failEma,gate5mFailRsi:d.failRsi,gate5mFailMacd:d.failMacd,gate5mFailAtr:d.failAtr,gate5m3Ema:d.threeOfFourEma,gate5m3Macd:d.threeOfFourMacd};
    for(const [id,v] of Object.entries(dmap))setDiagText(id,v);
  }
  ...
  function finalizeGateDiagnostics(analyzedRows){
    ...
    // V14.8: TRANSITION is informational, not an exclusive blocking gate.
    // A TRANSITION row continues through 15m setup -> 5m confirmation -> trigger -> R/R.
    ...
    for(const row of validRows){
      ...
      if(k==='confirm5m'){
        ...
        const timing=!Object.keys(checks).length&&(reason.includes('started before')||reason.includes('timestamp unavailable')||reason.includes('not pass'));
        if(timing)d.timing++;
        ...
        if(checks.ema&&checks.ema.pass===false)d.failEma++;
        ...
        if(passed===3&&checks.ema?.pass===false)d.threeOfFourEma++;
        if(passed===3&&checks.macd?.pass===false)d.threeOfFourMacd++;
      }
    }
    state.gateDiagnostics=g;renderGateDiagnostics();
    const d=g.confirm5mDetail;
    log(`Gate Diagnostics: Fast ${g.fastEvents} | Liquidity ${g.liquidityDrop} | OI ${g.oiDrop} | Data ${g.dataDrop} | Operational ${g.operationalDrop} | Entry ${g.entryAnalyzed} | EntryError ${g.entryError} | 1hBlock ${g.oneHourBlock} | 1hTransition(info) ${g.oneHourTransition} | Fresh/Elder ${g.freshnessElder} | 15m ${g.setup15m} | 5m ${g.confirm5m} [timing ${d.timing}; 2/4 ${d.score2}; 3/4 blocked ${d.score3}; closedVOLweak ${d.weakClosedVolume}; fail E/R/M/A ${d.failEma}/${d.failRsi}/${d.failMacd}/${d.failAtr}] | Trigger/Zone ${g.triggerZone} | R/R ${g.rr} | Soft ${g.softCaution} | SELECT ${g.select}`);
  }
...
function computePriceLocation(c1h,c15,c5,i15,step7,step8,{horizonMin=240,currentPriceOverride=NaN,currentTimeOverride=NaN}={}){
  ...
  const structuralUpsideAtr=openUpside?Infinity:(Number.isFinite(r)&&Number.isFinite(atr15)&&atr15>0?(r-currentPrice)/atr15:NaN);
  ...
  if(structuralAsymmetry===Infinity||Number.isFinite(structuralAsymmetry)){
    if(Number.isFinite(structuralAsymmetry)&&structuralAsymmetry<1)verdict='UNFAVORABLE';
...
function step9ChecksAt(candles,ind,i){
  ...
  // V14.10: the Fast Event is the live momentum trigger. The last completed 5m candle
  // only needs MACD support: either DIF is already above DEA OR histogram is improving.
  // A bearish veto exists only when both are unfavorable at the same time.
  ...
  // Fast Event already selects expansion. Very high ATR must not veto an otherwise valid
  // momentum confirmation; only abnormally LOW ATR remains a negative confirmation.
  const atrOk=atrPct>=V2_CONFIG.atrPercentileMin;
  return {
    // Closed-5m volume is diagnostic only. Live Fast Volume already measures participation
    // in the actual 60-second trigger and therefore avoids double-gating on stale volume.
    volume:{pass:volOk,required:false,current:candles[i].volume,sma20:volSma,previous:candles[i-1].volume},
    ema:{pass:emaOk,required:true,ema7:ind.ema7[i],ema25:ind.ema25[i]},
    rsi:{pass:rsiOk,required:false,value:rsi,rising:rsi>ind.rsi14[i-1]},
    macd:{pass:macdOk,required:true,dif,dea,hist,histRising},
    atr:{pass:atrOk,required:false,value:atr,percentile:atrPct}
  };
}

function analyzeStep9(candles,ind,step8){
  if(step8.status!=='PASS')return {status:'NOT_RUN',score:'0/4',passed:0,mandatoryPassed:false,reason:'Step 8 is not PASS',checks:{}};
  const t=candles.length-1,setupTime=Number(step8.setupTime);
  if(!Number.isFinite(setupTime))return {status:'WAIT',score:'0/4',passed:0,mandatoryPassed:false,reason:'Step 8 setup timestamp unavailable',checks:{}};
  if(!(Number(candles[t]?.openTime)>=setupTime))return {status:'WAIT',score:'0/4',passed:0,mandatoryPassed:false,reason:'latest completed 5m candle started before the 15m setup completed',checks:{}};
  const checks=step9ChecksAt(candles,ind,t);
  const qualityKeys=['ema','rsi','macd','atr'];
  const passed=qualityKeys.filter(k=>checks[k]?.pass).length;
  const mandatoryPassed=!!checks.ema?.pass&&!!checks.macd?.pass;
  const failed=qualityKeys.filter(k=>!checks[k]?.pass).map(k=>k.toUpperCase());
  // V14.10: 3/4 quality is sufficient when EMA and supportive MACD both pass.
  // Closed-candle volume is intentionally NOT part of the score because the live Fast Volume
  // already validates participation in the current Fast Event.
  const confirmed=passed>=3&&mandatoryPassed;
  const status=confirmed?'CONFIRMED':'WAIT';
  const volumeNote=checks.volume?.pass?'closed-5m VOL supportive':'closed-5m VOL weak (info only)';
  const reason=confirmed
    ?`current completed 5m ${passed}/4 confirmed; EMA + supportive MACD PASS; ${volumeNote}`
    :`current completed 5m is ${passed}/4${failed.length?` failed: ${failed.join(', ')}`:''}${mandatoryPassed?'':' | EMA/supportive MACD mandatory confirmation missing'}; ${volumeNote}`;
  return {status,score:`${passed}/4`,passed,mandatoryPassed,reason,checks,firstConfirmationIndex:confirmed?t:-1,firstConfirmationTime:confirmed?candles[t].closeTime:NaN};
}
...
function analyzeStep10(candles5,ind5,candles15,ind15,step8,step9,eventTime,currentPrice){
  ...
  if(step9.status!=='CONFIRMED')return {status:'WAIT',reason:'current completed 5m confirmation is not valid (need >=3/4 with EMA + supportive MACD PASS)'};
  ...
  // V14.5: Fast Event IS the expansion trigger. Measuring chase from the old 15m pullback
  // ceiling with a 0.50 ATR hard/core gate double-penalized valid breakouts. Keep the
  // extension as diagnostics; actual current-price entry quality is enforced by structural R/R.
  const extensionAtr=Number.isFinite(atr15)&&atr15>0&&Number.isFinite(step8.entryZoneHigh)
    ?(px-step8.entryZoneHigh)/atr15:NaN;
  return {status:'READY',reason:`valid 15m setup + current 5m ${step9.score} (EMA + supportive MACD mandatory) + current Fast Event trigger`,currentPrice:px,extensionAtr};
}
...
function finalizeEntryDecision(row,chart,finalSpread=NaN){
  ...
  // Auxiliary market-quality warnings: one warning alone does NOT cancel Entry Ready.
  // Two or more independent soft warnings demote SELECT to CAUTION.
  ...
  else if(chart.step7?.regime==='TRANSITION')softWarnings.push('1h TRANSITION');
  ...
  // WAIT_EXTENDED is diagnostic only in V14.5. Actual chase and reward/risk are enforced
  // later by Step 10 and structural R/R, so the same extension is not double-penalized.
  ...
  else if(s8.status!=='PASS')coreCautions.push(s8.reason||'15m setup not ready');

  if(s8.status==='PASS'){
    const passed=Number(chart.step9?.passed);
    if(chart.step9?.status!=='CONFIRMED'){
      if(Number.isFinite(passed)&&passed>=3)coreCautions.push(`${chart.step9.score} but EMA/supportive MACD mandatory confirmation missing`);
      else drops.push(`5m confirmation ${Number.isFinite(passed)?passed:0}/4`);
    }
  }
  ...
  if(s8.status==='PASS'&&chart.step9?.status==='CONFIRMED'){
    ...
    else if(chart.step10?.status==='SKIP_CHASE')coreCautions.push(chart.step10.reason); // legacy fallback
    else if(chart.step10?.status!=='READY')coreCautions.push(chart.step10?.reason||'entry trigger/zone not ready');
    else if(Number.isFinite(chart.step10?.extensionAtr)&&chart.step10.extensionAtr>1.50){
      softWarnings.push(`price extension ${fmt(chart.step10.extensionAtr,2)} ATR15 above setup zone`);
    }
  }
  ...
  if(chart.step10?.status==='READY'){
    ...
    else if(Number.isFinite(loc.structuralAsymmetry)&&loc.structuralAsymmetry<1)drops.push(`structural R/R ${fmt(loc.structuralAsymmetry,2)} < 1`);
  ...
  // V14.5: two independent warning FAMILIES demote to CAUTION. Correlated funding/crowding
  // or OI/taker warnings count once instead of being double-counted.
  if(softCategories.length>=2)return {result:'CAUTION',cautions:uniqueSoft,coreCautions:uniqueCore,softWarnings:uniqueSoft,softCategories,drops:uniqueDrops};
...
async function analyzeChartCandidate(row,serverTime,finalBookMap){
  try{
    ...
    const reason=`7:${step7.reason} | Fresh:${entryFreshnessReason(entryFreshness)} | Elder:${entryImpulseReason(entryImpulse)} | 8:${step8.reason} | 9:${step9.reason} | 10:${step10.reason}${priceLocation?` | Location:${priceLocation.verdict}; StructuralAsym=${priceLocation.structuralAsymmetry===Infinity?'OPEN':fmt(priceLocation.structuralAsymmetry,2)}; HistAsym=${fmt(priceLocation.historicalAsymmetry,2)}; StatN=${priceLocation.model.n}`:''}`;
    ...
    const reasons=[...(row.reasons||[]),...(decision.softWarnings||[]).map(x=>`entry soft: ${x}`),...(decision.coreCautions||[]).map(x=>`entry caution: ${x}`),...decision.drops.map(x=>`entry drop: ${x}`)];
...
function entryDecisionSummary(r){
  ...
  if(c.priceLocation?.structuralAsymmetry===Infinity)parts.push('R/R OPEN'); else if(Number.isFinite(c.priceLocation?.structuralAsymmetry))parts.push(`R/R ${fmt(c.priceLocation.structuralAsymmetry,2)}`);
  const cautions=c.decision?.cautions||[];
  const soft=c.decision?.softWarnings||[];
  if(r.result==='CAUTION'&&cautions.length)parts.push(cautions.slice(0,3).join(', '));
  else if(r.result==='SELECT'&&soft.length)parts.push(`soft: ${soft.slice(0,2).join(', ')}`);
  return parts.join(' | ');
}
...
function renderQueue(rows){
  ...
  el.innerHTML=q.map((r,i)=>{const c=r.chartAnalysis||v2NotRun('analysis unavailable');return `<span class="queueItem" title="${htmlEscape(c.reason||'')}"><b>${i+1}. ${htmlEscape(r.symbol)}</b> · 7 ${htmlEscape(c.step7?.regime||'—')} · 8 ${htmlEscape(c.step8?.setup||'—')} · 9 ${htmlEscape(c.step9?.score||'—')} · 10 ${htmlEscape(c.step10?.status||'—')} · Loc ${htmlEscape(c.priceLocation?.verdict||'—')} · R/R ${c.priceLocation?.structuralAsymmetry===Infinity?'OPEN':fmt(c.priceLocation?.structuralAsymmetry,2)} · <b>ENTRY READY</b></span>`;}).join('');
}
...
function logEntryDecisionDiagnostics(rows){
  ...
  const beforeOperationalCount=rows.length;
  rows=rows.filter(r=>{
  ...
  function exportJSON(){
    if(!state.results.length)return;
    const data={version:'CryptoOffer V11 — Coin Selection + Position Monitor',createdAt:new Date().toISOString(),settings:state.settings,snapshotElapsedMs:state.snapshotElapsedMs,context:state.context,thresholds:state.thresholds,priceLearning:{completed:state.priceLearning.filter(x=>x.completed).length,pending:state.priceLearning.filter(x=>!x.completed).length},gateDiagnostics:state.gateDiagnostics,results:state.results,requestCount:state.requestCount,errors:state.errors};
  ...
  window.CryptoFlowScanner=window.CryptoFlowScanner||{version:'V14.10',modules:{}};
  window.CryptoFlowScanner.version='V14.10';
