...
async function candidateDetails(row,refTime,serverTime,premiumMap,fundingInfoMap,thresholds){
  ...
  const [oldPx,oiHist,taker,freshOi,freshPremium,fastVolume]=await Promise.all([
  ...
  const essential=[
    return4h,rs4h,liveOiQty,liveOiNotional,volumeOiRatio,deltaOi,
    oiShort.delta5,oiShort.delta15,oiShort.p10_5m,liveSpread,liveDepth,
    fundingRate,premiumPct,takerRatio,timeToFunding,fundingProx
  ];
  if(essential.some(x=>!Number.isFinite(x))||fastVolume?.status==='UNAVAILABLE')return null;
  if(Number.isFinite(thresholds.spreadP90)&&liveSpread>thresholds.spreadP90)return null;
  if(Number.isFinite(thresholds.depthP10)&&liveDepth<thresholds.depthP10)return null;
  if(oiShort.severeDrop)return null;

  const fundingExtreme=Number.isFinite(thresholds.fundingP90)&&fundingRate>0&&fundingRate>thresholds.fundingP90;
  const premiumExtreme=Number.isFinite(thresholds.premiumP90)&&premiumPct>0&&premiumPct>thresholds.premiumP90;
  const oiGrowing=deltaOi>0;
  const oiLarge=Number.isFinite(thresholds.oiNotionalP75)&&liveOiNotional>=thresholds.oiNotionalP75;
  const takerLongBias=takerRatio>1;
  ...
  return {
    ...
    takerRatio,fundingRate,intervalHours,nextFunding,timeToFunding,fundingProx,fundingExposure,premiumPct,liveSpread,liveDepth,crowding,
    baseResult:result,result,reasons
  };
}
...
function finalizeEntryDecision(row,chart){
  ...
  if(row.baseResult==='CAUTION')cautions.push('base scanner has soft caution(s)');
...
async function analyzeChartCandidate(row,serverTime){
  try{
    const [c1h,c15,c5,finalBook]=await Promise.all([
      ...
      fetchJson(`${BASE}/fapi/v1/ticker/bookTicker?symbol=${encodeURIComponent(row.symbol)}`,{retries:2,timeout:12000})
    ]);
...
function updateSummary(rows,universeCount,fastCount,eventCount){
  ...
  const analyzedRows=await mapLimit(rows,2,async r=>analyzeChartCandidate(r,finalServerTime),{
  ...
  window.CryptoFlowScanner=window.CryptoFlowScanner||{version:'V14.0',modules:{}};
  window.CryptoFlowScanner.version='V14.0';
