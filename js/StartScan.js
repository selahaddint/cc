(() => {
  ...
  function finalizeGateDiagnostics(analyzedRows){
    ...
    const d=g.confirm5mDetail;
    log(`Gate Diagnostics: Fast ${g.fastEvents} | Liquidity ${g.liquidityDrop} | OI ${g.oiDrop} | Data ${g.dataDrop} | Operational ${g.operationalDrop} | Entry ${g.entryAnalyzed} | EntryError ${g.entryError} | 1hBlock ${g.oneHourBlock} | 1hTransition(info) ${g.oneHourTransition} | Fresh/Elder ${g.freshnessElder} | 15m ${g.setup15m} | 5m ${g.confirm5m} [timing ${d.timing}; 2/4 ${d.score2}; 3/4 ${d.score3}; closedVOLweak ${d.weakClosedVolume}; fail E/R/M/A ${d.failEma}/${d.failRsi}/${d.failMacd}/${d.failAtr}] | Trigger/Zone ${g.triggerZone} | R/R ${g.rr} | Soft ${g.softCaution} | SELECT ${g.select}`);
  }
...
function analyzeStep9(candles,ind,step8){
  if(step8.status!=='PASS')return {status:'NOT_RUN',score:'0/4',passed:0,mandatoryPassed:false,timingPending:false,reason:'Step 8 is not PASS',checks:{}};
  const t=candles.length-1,setupTime=Number(step8.setupTime);
  if(!Number.isFinite(setupTime))return {status:'WAIT',score:'—',passed:NaN,mandatoryPassed:false,timingPending:true,reason:'Step 8 setup timestamp unavailable',checks:{}};
  // Timing is not an evaluated 5m failure. Until a completed 5m candle exists at/after
  // the valid 15m setup, keep the candidate as CAUTION instead of misclassifying it as 0/4.
  if(!(Number(candles[t]?.openTime)>=setupTime))return {status:'WAIT',score:'—',passed:NaN,mandatoryPassed:false,timingPending:true,reason:'latest completed 5m candle is not ready after the 15m setup',checks:{}};
  ...
  return {status,score:`${passed}/4`,passed,mandatoryPassed,timingPending:false,reason,checks,firstConfirmationIndex:confirmed?t:-1,firstConfirmationTime:confirmed?candles[t].closeTime:NaN};
}
  ...
  if(s8.status==='PASS'){
    ...
    if(chart.step9?.status!=='CONFIRMED'){
      if(chart.step9?.timingPending){
        coreCautions.push(chart.step9?.reason||'5m candle not ready');
      }else if(Number.isFinite(passed)&&passed>=2){
        coreCautions.push(`${chart.step9.score} not Entry Ready${chart.step9?.mandatoryPassed?'':' — EMA/supportive MACD missing'}`);
      }else{
        drops.push(`5m confirmation ${Number.isFinite(passed)?passed:0}/4 too weak`);
      }
    }
  }
...
function logEntryDecisionDiagnostics(rows){
  ...
  window.CryptoFlowScanner=window.CryptoFlowScanner||{version:'V14.12',modules:{}};
  window.CryptoFlowScanner.version='V14.12';
