(() => {
  ...
  function saveScanSettings(){
    ...
    const settings={
      windowSec:Number.isFinite(windowSec)?windowSec:15,
      minIncrease:Number.isFinite(minIncrease)?minIncrease:1.5,
      maxIncrease:Number.isFinite(maxIncrease)?maxIncrease:5,
      autoScan
    };
  ...
  function loadScanSettings(){
    try{
      ...
      const windowSec=Number(saved?.windowSec);
      const minIncrease=Number(saved?.minIncrease);
      const maxIncrease=Number(saved?.maxIncrease);
  ...
  function mergeRowsWithFollow(rows){
    ...
    if(!preserved.length)return fresh.slice();
    const freshBySymbol=new Map(fresh.map(r=>[r.symbol,r]));
    const used=new Set(),out=[];
    for(const oldRow of preserved){
      const latest=freshBySymbol.get(oldRow.symbol);
      if(latest){
        if(!latest.decisionSupport&&oldRow.decisionSupport)latest.decisionSupport=oldRow.decisionSupport;
        out.push(latest);used.add(latest.symbol);
      }else{
        out.push(oldRow);used.add(oldRow.symbol);
      }
    }
    for(const r of fresh)if(!used.has(r.symbol))out.push(r);
    return out;
  }
  ...
  function resetUI(){
    ...
    if(preservedFollowRows().length)renderCandidates([],state.settings?.windowSec||num($('windowSec').value)||15);
  ...
  function gridRowsForDisplay(rows){
    let cautionCount=0,waitCount=0;
    return (Array.isArray(rows)?rows:[]).filter(r=>{
      if(r?.result==='CAUTION')return cautionCount++<3;
      if(r?.result==='WAIT')return waitCount++<3;
      return true;
    });
  }

  function renderCandidates(rows,windowSec){
    const body=$('candidateBody'),displayRows=mergeRowsWithFollow(gridRowsForDisplay(rows));
