...
function analyzeStep9(candles,ind,step8){
  ...
  // V14.4: 4/5 is enough only when EMA and MACD are both positive.
...
function analyzeStep10(candles5,ind5,candles15,ind15,step8,step9,eventTime,currentPrice){
  ...
  return {status:'READY',reason:`valid 15m setup + current 5m ${step9.score} (EMA+MACD mandatory) + current Fast Event trigger + price inside allowed entry range`,currentPrice:px};
}
...
function finalizeEntryDecision(row,chart,finalSpread=NaN){
  ...
  // WAIT_EXTENDED is diagnostic only in V14.4. Actual chase and reward/risk are enforced
  ...
  if(s8.status==='PASS'&&chart.step9?.status==='CONFIRMED'){
    ...
    else if(chart.step10?.status==='SKIP_CHASE')coreCautions.push(chart.step10.reason);
  ...
  if(uniqueDrops.length)return {result:'DROP',cautions:[...uniqueCore,...uniqueSoft],coreCautions:uniqueCore,softWarnings:uniqueSoft,drops:uniqueDrops};
  if(uniqueCore.length)return {result:'CAUTION',cautions:[...uniqueCore,...uniqueSoft],coreCautions:uniqueCore,softWarnings:uniqueSoft,drops:uniqueDrops};
  if(uniqueSoft.length>=2)return {result:'CAUTION',cautions:uniqueSoft,coreCautions:uniqueCore,softWarnings:uniqueSoft,drops:uniqueDrops};
  return {result:'SELECT',cautions:[],coreCautions:uniqueCore,softWarnings:uniqueSoft,drops:uniqueDrops};
}
...
function logEntryDecisionDiagnostics(rows){
  ...
  window.CryptoFlowScanner=window.CryptoFlowScanner||{version:'V14.4',modules:{}};
  window.CryptoFlowScanner.version='V14.4';
