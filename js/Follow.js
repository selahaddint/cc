(() => {
  ...
  const FOLLOW_CONFIG=Object.freeze({refreshMs:60*1000,atrBreakExit:0.50,trailingAtrMultiple:2,breakEvenArmAtr:1,maxHoldMs:FOLLOW_MAX_HOLD_MS});
  ...
  // Best-effort EXIT sound. Audio is deliberately isolated from the trading/Follow logic:
  // unsupported/blocked/broken audio must never throw into, await, or delay the monitor.
  let followAudioContext=null;
  let followAudioUnavailableLogged=false;

  function followGetAudioContext(){
    try{
      const AudioCtx=window.AudioContext||window.webkitAudioContext;
      if(!AudioCtx)return null;
      if(!followAudioContext||followAudioContext.state==='closed')followAudioContext=new AudioCtx();
      return followAudioContext;
    }catch(e){
      if(!followAudioUnavailableLogged){followAudioUnavailableLogged=true;followLog(`EXIT sound unavailable: ${e.message||e}. Follow continues normally.`);}
      return null;
    }
  }

  // Call only from a real user gesture (the Follow button click) so mobile Safari/Chrome
  // have the best chance to permit later EXIT sounds. This function never blocks Follow.
  function unlockFollowExitAudio(){
    try{
      const ctx=followGetAudioContext();
      if(!ctx)return false;
      const prime=()=>{
        try{
          ...
          if(!followAudioUnavailableLogged){followAudioUnavailableLogged=true;followLog(`EXIT sound unlock failed: ${e.message||e}. Follow continues normally.`);}
        }
      };
      if(ctx.state==='suspended'){
        const resumed=ctx.resume();
        if(resumed&&typeof resumed.then==='function')resumed.then(prime).catch(e=>{
          if(!followAudioUnavailableLogged){followAudioUnavailableLogged=true;followLog(`EXIT sound is blocked by the browser: ${e.message||e}. Follow continues normally.`);}
        });
        else prime();
      }else prime();
      return true;
    }catch(e){
      if(!followAudioUnavailableLogged){followAudioUnavailableLogged=true;followLog(`EXIT sound setup failed: ${e.message||e}. Follow continues normally.`);}
      return false;
    }
  }

  function playFollowExitBeep(symbol){
    try{
      const ctx=followGetAudioContext();
      if(!ctx)return false;
      const schedule=()=>{
        try{
          ...
          [0,0.22,0.44].forEach((offset,index)=>{
            const start=base+offset,end=start+0.15;
            const osc=ctx.createOscillator(),gain=ctx.createGain();
            osc.type='sine';
            osc.frequency.setValueAtTime(index===1?1046:880,start);
          ...
          followLog(`EXIT sound played: ${symbol}.`);
        }catch(e){
          if(!followAudioUnavailableLogged){followAudioUnavailableLogged=true;followLog(`EXIT sound failed: ${e.message||e}. Follow continues normally.`);}
        }
      };
      if(ctx.state==='suspended'){
        const resumed=ctx.resume();
        if(resumed&&typeof resumed.then==='function')resumed.then(schedule).catch(e=>{
          if(!followAudioUnavailableLogged){followAudioUnavailableLogged=true;followLog(`EXIT sound was blocked: ${e.message||e}. Follow continues normally.`);}
        });
        else schedule();
      }else schedule();
      return true;
    }catch(e){
      if(!followAudioUnavailableLogged){followAudioUnavailableLogged=true;followLog(`EXIT sound error: ${e.message||e}. Follow continues normally.`);}
      return false;
    }
  }
  ...
  function setLiveUI(symbol,changeText='',resultText=''){
    ...
    if(result){
      ...
      if(resultText.includes('PROTECT')||resultText.includes('OICizgiAnalysis BEARISH'))result.classList.add('warn');
  ...
  // === OICizgiAnalysis START ===
  // OI is confirmation/context only. It must never create EXIT by itself.
  function OICizgiAnalysis(rawRows){
    ...
    if(rows.length<7)return {ready:false,bearish:false,lineDown:false,barBelowPreviousPeak:false,latestOi:NaN,previousPeakOi:NaN};
    ...
    const bearish=!!peak&&lineDown&&barBelowPreviousPeak;
    return {ready:!!peak,bearish,lineDown,barBelowPreviousPeak,latestOi:last.oi,latestValue:last.value,previousValue:prev.value,previousPeakOi:peak?.oi??NaN,previousPeakTime:peak?.timestamp??NaN,timestamp:last.timestamp};
  }
  // === OICizgiAnalysis END ===
  ...
  async function refreshLive5mContext(task,serverTime){
    ...
    try{
      ...
      return task.live5m||{bucket,lsa:{value:NaN,arrow:'—'},lsp:{value:NaN,arrow:'—'},gls:{value:NaN,arrow:'—'},taker5:{value:NaN,arrow:'—'},oiHist:[],oiCizgi:{ready:false,bearish:false}};
    }
  }
  ...
  function buildLiveFollowAnalysis(task,{serverTime,currentPrice,currentOi,premiumRaw,closed1m,live5m}){
    ...
    const oiC=live5m?.oiCizgi||{ready:false,bearish:false};
    if(oiC.bearish&&decision==='HOLD'){
      label='OI STRUCTURE WEAKENING';
      decision='PROTECT';
    }
    const oiCText=oiC.bearish?'OICizgiAnalysis BEARISH':oiC.ready?'OICizgiAnalysis OK':'OICizgiAnalysis CAL';
  ...
  async function refreshFollow(symbol){
    ...
    try{
      ...
      // Use only candles that STARTED after the previous check. Using closeTime here
      // would re-include price action that happened before Follow/lastCheck inside the same 1m candle.
      const active1m=recent1m.filter(c=>c.openTime>=since);
      ...
      // This level existed throughout the just-finished observation interval.
      // It can therefore be safely checked against that interval's observed Low.
      const previousEffectiveExitLevel=task.effectiveExitLevel;
      ...
      if(Number.isFinite(task.lastHL)&&atr15>0){
        if(last15.close<task.lastHL){
          ...
          else yellow.push(`15m close below HL: ${followFmt(breakRatio,2)} ATR — PROTECT`);
      ...
      // The bearish price-action state persists until a clean new HH invalidates it.
      if(pts15.high&&pts15.high.time>Number(task.lastSeenHighTime||0)){
        ...
        if(Number.isFinite(oldHH)){
          if(newHigh.price<=oldHH){
            task.priceActionBearish=true;
            task.priceActionReason=`15m failed HH: ${followPriceFmt(newHigh.price)} ≤ ${followPriceFmt(oldHH)}`;
          }else{
            ...
            if(rsiDiv||macdDiv){
              task.priceActionBearish=true;
              task.priceActionReason=`15m bearish divergence${rsiDiv&&macdDiv?' (RSI + MACD-H)':rsiDiv?' (RSI)':' (MACD-H)'}`;
            }else{
              task.priceActionBearish=false;
              task.priceActionReason='';
            }
            task.lastHH=newHigh.price;task.lastHHTime=newHigh.time;task.lastHHRSI=newRSI;task.lastHHMACD=newMACD;
          }
        }else{
          task.priceActionBearish=false;task.priceActionReason='';
          task.lastHH=newHigh.price;task.lastHHTime=newHigh.time;task.lastHHRSI=newRSI;task.lastHHMACD=newMACD;
        }
      }
      if(task.priceActionBearish&&task.priceActionReason)yellow.push(task.priceActionReason);

      // 12) Volume is confirmation only.
      const sellingVolume=sellingVolumeExpansion(task.tf5?.candles)||sellingVolumeExpansion(c15);
      if(sellingVolume)confirm.push('selling volume expanding');

      // 13) OI is confirmation only; direction is not inferred from OI alone.
      const priceOiBearish=Number.isFinite(task.prevPrice)&&Number.isFinite(task.prevOi)&&Number.isFinite(currentOi)&&currentPrice<task.prevPrice&&currentOi>task.prevOi;
      if(priceOiBearish)confirm.push('price ↓ + OI ↑');

      // 14–15) Highest price + ATR trailing + break-even protection.
      // Entry ATR is frozen once, while the trailing level ratchets upward only.
      if(atr15>0){
        if(!Number.isFinite(task.entryAtr15))task.entryAtr15=atr15;
        const rawTrailingLevel=task.highest-FOLLOW_CONFIG.trailingAtrMultiple*atr15;
        task.trailingLevel=Number.isFinite(task.trailingLevel)?Math.max(task.trailingLevel,rawTrailingLevel):rawTrailingLevel;

        // Break-even arms after +1 x ENTRY ATR, then remains armed for this Follow session.
        if(!task.breakEvenArmed&&Number.isFinite(task.entryAtr15)&&task.highest>=task.entryPrice+FOLLOW_CONFIG.breakEvenArmAtr*task.entryAtr15){
          task.breakEvenArmed=true;task.breakEvenArmedAt=serverTime;
        }

        const effectiveExitLevel=task.breakEvenArmed?Math.max(task.entryPrice,task.trailingLevel):task.trailingLevel;
        ...
        // No candle-close wait. Preserve event order:
        // - the previous effective level existed during the observed interval, so compare it with observedLow;
        // - a newly raised level only exists now, so compare the current price with the new level.
        const previousLevelTouched=Number.isFinite(previousEffectiveExitLevel)&&observedLow<=previousEffectiveExitLevel;
        const currentLevelTouched=Number.isFinite(effectiveExitLevel)&&currentPrice<=effectiveExitLevel;
        if(previousLevelTouched||currentLevelTouched){
          const hitLevel=previousLevelTouched?previousEffectiveExitLevel:effectiveExitLevel;
          const hitPrice=previousLevelTouched?observedLow:currentPrice;
          if(task.breakEvenArmed&&task.entryPrice>=hitLevel){
            red.push(`Break-even protection hit: ${previousLevelTouched?'1m low':'Current'} ${followPriceFmt(hitPrice)} ≤ ${followPriceFmt(hitLevel)} after +${followFmt(FOLLOW_CONFIG.breakEvenArmAtr,2)} Entry ATR profit`);
          }else{
            red.push(`ATR trailing hit: ${previousLevelTouched?'1m low':'Current'} ${followPriceFmt(hitPrice)} ≤ ${followPriceFmt(hitLevel)} (Highest ${followPriceFmt(task.highest)})`);
          }
        }
      }
      ...
      // 17) Live OI/flow analysis. It may create PROTECT/confirmation but never EXIT by itself.
      ...
      const oiStructureBearish=!!live.oiCizgi?.bearish;
      if(oiStructureBearish){
        yellow.push(`OI structure weakening: value line DOWN + latest OI ${followCompact(live.oiCizgi.latestOi)} < previous peak ${followCompact(live.oiCizgi.previousPeakOi)}`);
        confirm.push('OI structure bearish');
      }

      // 18) FINAL PROTECT -> EXIT combination.
      // Main bearish condition: 15m EMA DOWN + MACD histogram DOWN.
      // It becomes EXIT only with at least one independent confirmation group:
      //   A) 5m LL/LH + 3m LL/LH, OR
      //   B) persistent failed-HH / bearish-divergence state, OR
      //   C) bearish flow/volume/OI confirmation.
      const threeBearish=!!task.tf3?.structure?.bearish;
      const shortStructureConfirm=fiveBearish&&threeBearish;
      const priceActionConfirm=!!task.priceActionBearish;
      const flowConfirm=sellingVolume||priceOiBearish||oiStructureBearish||!!live.strongSeller||!!live.fastUnwind||!!live.sellBuild;
      const bearishImpulse=emaDown&&macdDown;

      if(bearishImpulse){
        const groups=[];
        if(shortStructureConfirm)groups.push('5m+3m LL/LH');
        if(priceActionConfirm)groups.push(task.priceActionReason||'15m bearish price action');
        if(flowConfirm){
          const flowReasons=[];
          if(sellingVolume)flowReasons.push('selling volume');
          if(priceOiBearish)flowReasons.push('price↓ + OI↑');
          if(oiStructureBearish)flowReasons.push('OI structure bearish');
          if(live.strongSeller)flowReasons.push('strong seller');
          if(live.fastUnwind)flowReasons.push('fast unwind');
          if(live.sellBuild)flowReasons.push('sell pressure');
          groups.push(`flow: ${flowReasons.join(', ')}`);
        }
        if(groups.length)red.push(`PROTECT→EXIT: 15m EMA↓ + MACD-H↓ confirmed by ${groups.join(' | ')}`);
      }
      ...
      if(status==='RED'&&previousStatus!=='RED'){
        // Audio is fire-and-forget: any browser/audio failure is contained inside the sound helper.
        playFollowExitBeep(symbol);
        void showFollowExitNotification(symbol,currentPrice,reason);
      }
  ...
  async function startFollow(symbol){
    ...
    // Unlock audio immediately inside the user's click gesture. It is intentionally non-blocking
    // and any failure is contained so Follow can continue on every browser/computer.
    unlockFollowExitAudio();
    // Follow button click is also the user gesture used to request notification permission.
    // Failure/denial never blocks the Follow algorithm.
    await ensureFollowNotificationPermission();
    const now=Date.now();
    const task={symbol,entryPrice:entry,stopPrice:stop,startedAt:now,highest:entry,entryAtr15:NaN,trailingLevel:NaN,breakEvenArmed:false,breakEvenArmedAt:0,effectiveExitLevel:NaN,priceActionBearish:false,priceActionReason:'',status:'GREEN',reason:'Follow starting',refreshing:false,prevPrice:NaN,prevOi:NaN,prevBasisPct:NaN,prevFundingRate:NaN,prevLiveTime:NaN,oiDeltaHistory:[],live5m:null,liveAnalysis:null,lastHL:NaN,lastHH:NaN,lastSeenLowTime:0,lastSeenHighTime:0,tf15:null,tf5:null,tf3:null,timer:null,rowSnapshot:sourceRow,currentPrice:NaN,currentOi:NaN,lastCheck:now};
