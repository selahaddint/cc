(() => {
  ...
  function mergeRowsWithFollow(rows){
    ...
    // Follow rows also do not consume the CAUTION/WAIT first-3 display quota.
  ...
  async function analyzeEntryFreshness(symbol,currentPrice,serverTime){
    ...
    try{
      ...
      const value=entryValueExtensionFeature(recent.candles,recent.ind,t,currentPrice);
      if(!value)return {status:'UNAVAILABLE',ready:false,n:0,impulse,reason:'15m freshness inputs incomplete'};
      const current=hl?entryFreshnessFeature(recent.candles,recent.ind,t,hl.price,currentPrice):null;
      const hlPrice=hl?.price??NaN,hlExtension=current?.hlExtension??NaN;

      // Early-selection deterministic no-chase gate. Active HL is useful context but is NOT a hard SELECT gate.
      // Historical analogs are required only after price extends > 0.50 ATR15 above the value-zone ceiling.
      if(value.valueExtension<=V2_CONFIG.noChaseAtr15){
        return {status:'FRESH',ready:true,n:0,impulse,hlPrice,hlExtension,valueExtension:value.valueExtension,
          ...
          reason:value.valueExtension<=0?'price is inside/at 15m value zone':`15m value extension ${value.valueExtension.toFixed(2)} ATR <= ${V2_CONFIG.noChaseAtr15.toFixed(2)} ATR early-select limit${hl?'':' (no active HL required)'}`};
      }
      ...
      if(!hl||!current){
        return {status:'WAIT_EXTENDED',ready:false,n:0,impulse,needsModel:false,refreshNeeded:false,hlPrice,hlExtension,valueExtension:value.valueExtension,
          medianMfeAtr:NaN,medianMaeAtr:NaN,asymmetry:NaN,reason:`15m value extension ${value.valueExtension.toFixed(2)} ATR > ${V2_CONFIG.noChaseAtr15.toFixed(2)} ATR and no active confirmed HL exists for historical analog matching`};
      }

      const cached=entryFreshnessCachedModel(symbol);
      if(!cached){
        return {status:'WAIT_EXTENDED',ready:false,n:0,impulse,needsModel:true,refreshNeeded:false,hlPrice:hl.price,hlExtension:current.hlExtension,valueExtension:current.valueExtension,
          medianMfeAtr:NaN,medianMaeAtr:NaN,asymmetry:NaN,reason:`15m value extension ${current.valueExtension.toFixed(2)} ATR > ${V2_CONFIG.noChaseAtr15.toFixed(2)} ATR; historical model not cached yet`};
      }

      const neighbors=selectIndependentFreshnessNeighbors(cached.samples,current,hl.time),n=neighbors.length;
      if(n<cfg.minSamples){
        return {status:'WAIT_EXTENDED',ready:false,n,impulse,needsModel:false,refreshNeeded:cached.refreshNeeded,hlPrice:hl.price,hlExtension:current.hlExtension,valueExtension:current.valueExtension,
          medianMfeAtr:NaN,medianMaeAtr:NaN,asymmetry:NaN,reason:`15m value extension ${current.valueExtension.toFixed(2)} ATR; independent cached analogs ${n}/${cfg.minSamples}`};
      }
  ...
  async function candidateDetails(row,refTime,serverTime,premiumMap,fundingInfoMap,thresholds){
    ...
    const [oldPx,oiHist,taker,freshOi,freshPremium,fastVolume,trend1h]=await Promise.all([
      ...
      analyzeFastVolume(symbol,serverTime),
      analyzeSelectTrend1h(symbol,serverTime)
    ]);
    ...
    const decisionPrice=Number.isFinite(mid)&&mid>0?mid:now;
    ...
    if(!Number.isFinite(return4h)||!Number.isFinite(rs4h)||!Number.isFinite(liveOiQty)||!Number.isFinite(liveOiNotional)||!Number.isFinite(volumeOiRatio)||!Number.isFinite(deltaOi)||!Number.isFinite(oiShort.delta5)||!Number.isFinite(oiShort.delta15)||!Number.isFinite(oiShort.p10_5m)||!Number.isFinite(liveSpread)||!Number.isFinite(liveDepth)||!Number.isFinite(fundingRate)||!Number.isFinite(premiumPct)||!Number.isFinite(takerRatio)||!Number.isFinite(timeToFunding)||!Number.isFinite(fundingProx)||fastVolume?.status==='UNAVAILABLE'||trend1h?.status==='UNAVAILABLE'){
    ...
    if(result!=='WAIT'){
      ...
      if(oiShort.severeDrop){result='WAIT';reasons.push(`OI 5m severe drop ${pct(oiShort.delta5)} <= own severe threshold ${pct(oiShort.severeThreshold)} (P10 ${pct(oiShort.p10_5m)})`);}
      if(trend1h?.regime==='DOWNTREND'){result='WAIT';reasons.push(`1h explicit DOWNTREND — ${trend1h.reason}`);}
    }
    if(result==='SELECT'){
      const cautions=[];
      ...
      // General Context is informational only. UP / DOWN / MIXED must never
      // promote or demote SELECT / CAUTION / WAIT by itself.
      ...
      if(cautions.length){
        result='CAUTION';reasons.push(...cautions);
        if(marketContext)reasons.push(`market ${marketContext} context (info only)`);
      }else{
        if(marketContext)reasons.push(`market ${marketContext} context (info only)`);
        reasons.push(`OI 5m ${pct(oiShort.delta5)} / 15m ${pct(oiShort.delta15)} — short-term OI acceptable`);
        reasons.push(`fast volume ${fastVolume.status} — rank ${fmt(fastVolume.rank,0)}p, ratio ${fmt(fastVolume.ratio,2)}x`);
        reasons.push(`1h ${trend1h.regime} — ${trend1h.reason}`);
        reasons.push('all scanner checks aligned');
      }
    }
    ...
    if(result==='SELECT'){
      entryFreshness=await analyzeEntryFreshness(symbol,decisionPrice,serverTime);
      entryImpulse=entryFreshness?.impulse||null;
      const freshnessText=entryFreshnessReason(entryFreshness);
      if(entryFreshness?.status==='WAIT_EXTENDED'||entryFreshness?.status==='CALIBRATING'||entryFreshness?.status==='UNAVAILABLE'){
        result='WAIT';
        ...
        if(entryImpulse?.status==='BEARISH'||entryImpulse?.status==='UNAVAILABLE'||!entryImpulse){
          result='WAIT';
    ...
    return {...row,decisionPrice,oiQty:liveOiQty,oiNotional:liveOiNotional,volumeOiRatio,return4h,rs4h,deltaOi,deltaOi5:oiShort.delta5,deltaOi15:oiShort.delta15,oi5P10:oiShort.p10_5m,oi5SevereThreshold:oiShort.severeThreshold,oiShortStatus:oiShort.status,fastVolumeStatus:fastVolume?.status||'UNAVAILABLE',fastVolumeRatio:fastVolume?.ratio??NaN,fastVolumeRank:fastVolume?.rank??NaN,trend1hRegime:trend1h?.regime||'UNAVAILABLE',trend1hReason:trend1h?.reason||'unavailable',takerRatio,fundingRate,intervalHours,nextFunding,timeToFunding,fundingProx,fundingExposure,premiumPct,liveSpread,liveDepth,crowding,entryFreshness,entryImpulse,result,reasons};
  }
  ...
  function gridRowsForDisplay(rows){
    ...
    // Grid display contract:
    //   1) show EVERY SELECT
    //   2) show at most 3 CAUTION
    //   3) show at most 3 WAIT
    //   4) do not show REJECT/other statuses in the response grid
    // Keep these groups explicit so a CAUTION/WAIT display cap can never hide SELECT rows.
    const selects=source.filter(r=>r?.result==='SELECT');
    const cautions=source.filter(r=>r?.result==='CAUTION').slice(0,3);
    const waits=source.filter(r=>r?.result==='WAIT').slice(0,3);
    return [...selects,...cautions,...waits];
  }
  ...
  async function startScan(){
    ...
    try{
      ...
      log(`Result source before grid: SELECT ${rows.filter(r=>r.result==='SELECT').length} | CAUTION ${rows.filter(r=>r.result==='CAUTION').length} | WAIT ${rows.filter(r=>r.result==='WAIT').length} | REJECT ${rows.filter(r=>r.result==='REJECT').length}. Grid shows all SELECT + max 3 CAUTION + max 3 WAIT.`);
  ...
  window.CryptoFlowScanner=window.CryptoFlowScanner||{version:'V13.5',modules:{}};
  window.CryptoFlowScanner.version='V13.5';
