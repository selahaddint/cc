...
async function candidateDetails(row,refTime,serverTime,premiumMap,fundingInfoMap,thresholds,bookMap){
  ...
  if(Number.isFinite(thresholds.depthP10)&&liveDepth<thresholds.depthP10){recordBaseDrop('depth < p10');return null;}
...
function entryDecisionSummary(r){
  ...
  if(c.step9?.score)parts.push(`5m ${c.step9.score}`);
  if(c.step10?.status&&c.step10.status!=='NOT_RUN')parts.push(c.step10.status);
...
function logEntryDecisionDiagnostics(rows){
  ...
  window.CryptoFlowScanner=window.CryptoFlowScanner||{version:'V14.1',modules:{}};
  window.CryptoFlowScanner.version='V14.1';
