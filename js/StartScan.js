(() => {
  ...
  function emptyGateDiagnostics(){return {fastEvents:0,liquidityDrop:0,oiDrop:0,dataDrop:0,operationalDrop:0,entryAnalyzed:0,entryError:0,oneHourBlock:0,oneHourTransition:0,freshnessElder:0,setup15m:0,confirm5m:0,triggerZone:0,rr:0,softCaution:0,select:0};}
  ...
  function classifyEntryPrimaryGate(row){
    ...
    if(c.step7?.regime==='TRANSITION')return 'oneHourTransition';
  ...
  function finalizeGateDiagnostics(analyzedRows){
    const g=state.gateDiagnostics||emptyGateDiagnostics();
    g.entryAnalyzed=(analyzedRows||[]).filter(Boolean).length;
    for(const k of ['entryError','oneHourBlock','oneHourTransition','freshnessElder','setup15m','confirm5m','triggerZone','rr','softCaution','select'])g[k]=0;
    for(const row of (analyzedRows||[]).filter(Boolean)){const k=classifyEntryPrimaryGate(row);g[k]=(g[k]||0)+1;}
    state.gateDiagnostics=g;renderGateDiagnostics();
    log(`Gate Diagnostics: Fast ${g.fastEvents} | Liquidity ${g.liquidityDrop} | OI ${g.oiDrop} | Data ${g.dataDrop} | Operational ${g.operationalDrop} | Entry ${g.entryAnalyzed} | EntryError ${g.entryError} | 1hBlock ${g.oneHourBlock} | 1hTransition ${g.oneHourTransition} | Fresh/Elder ${g.freshnessElder} | 15m ${g.setup15m} | 5m ${g.confirm5m} | Trigger/Zone ${g.triggerZone} | R/R ${g.rr} | Soft ${g.softCaution} | SELECT ${g.select}`);
  }
...
function finalizeEntryDecision(row,chart,finalSpread=NaN){
  ...
  else if(chart.step7?.regime==='TRANSITION')coreCautions.push('1h TRANSITION');
...
function logEntryDecisionDiagnostics(rows){
  ...
  window.CryptoFlowScanner=window.CryptoFlowScanner||{version:'V14.7',modules:{}};
  window.CryptoFlowScanner.version='V14.7';
