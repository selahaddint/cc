(() => {
  ...
  function renderGateDiagnostics(){
    ...
    const dmap={gate5mTiming:d.timing,gate5mScore0:d.score0,gate5mScore1:d.score1,gate5mScore2:d.score2,gate5mScore3:d.score3,gate5mScore4:d.score4,gate5mFailVol:d.failVolume,gate5mFailEma:d.failEma,gate5mFailRsi:d.failRsi,gate5mFailMacd:d.failMacd,gate5mFailAtr:d.failAtr,gate5m4Ema:d.fourOfFiveEma,gate5m4Macd:d.fourOfFiveMacd};
  ...
  function finalizeGateDiagnostics(analyzedRows){
    ...
    g.confirm5mDetail={timing:0,score0:0,score1:0,score2:0,score3:0,score4:0,failVolume:0,failEma:0,failRsi:0,failMacd:0,failAtr:0,fourOfFiveEma:0,fourOfFiveMacd:0};
    for(const row of validRows){
      ...
      if(k==='confirm5m'){
        ...
        if(Number.isFinite(passed)){
          if(passed<=0)d.score0++;else if(passed===1)d.score1++;else if(passed===2)d.score2++;else if(passed===3)d.score3++;else if(passed===4)d.score4++;
        }
        if(checks.volume&&checks.volume.pass===false)d.failVolume++;
        ...
        if(passed===4&&checks.ema?.pass===false)d.fourOfFiveEma++;
        if(passed===4&&checks.macd?.pass===false)d.fourOfFiveMacd++;
      }
    }
    state.gateDiagnostics=g;renderGateDiagnostics();
    const d=g.confirm5mDetail;
    log(`Gate Diagnostics: Fast ${g.fastEvents} | Liquidity ${g.liquidityDrop} | OI ${g.oiDrop} | Data ${g.dataDrop} | Operational ${g.operationalDrop} | Entry ${g.entryAnalyzed} | EntryError ${g.entryError} | 1hBlock ${g.oneHourBlock} | 1hTransition(info) ${g.oneHourTransition} | Fresh/Elder ${g.freshnessElder} | 15m ${g.setup15m} | 5m ${g.confirm5m} [timing ${d.timing}; 3/5 ${d.score3}; 4/5 ${d.score4}; fail V/E/R/M/A ${d.failVolume}/${d.failEma}/${d.failRsi}/${d.failMacd}/${d.failAtr}] | Trigger/Zone ${g.triggerZone} | R/R ${g.rr} | Soft ${g.softCaution} | SELECT ${g.select}`);
  }
...
function analyzeStep9(candles,ind,step8){
  if(step8.status!=='PASS')return {status:'NOT_RUN',score:'0/5',passed:0,mandatoryPassed:false,reason:'Step 8 is not PASS',checks:{}};
  const t=candles.length-1,setupTime=Number(step8.setupTime);
  if(!Number.isFinite(setupTime))return {status:'WAIT',score:'0/5',passed:0,mandatoryPassed:false,reason:'Step 8 setup timestamp unavailable',checks:{}};
  if(!(Number(candles[t]?.openTime)>=setupTime))return {status:'WAIT',score:'0/5',passed:0,mandatoryPassed:false,reason:'latest completed 5m candle started before the 15m setup completed',checks:{}};
  ...
  // V14.5: 4/5 is enough only when EMA and MACD are both positive.
  // This keeps directional momentum mandatory while avoiding a zero-SELECT intersection
  // caused by requiring every auxiliary metric to be perfect on the same 5m close.
  const confirmed=passed>=4&&mandatoryPassed;
  const status=confirmed?'CONFIRMED':'WAIT';
  const reason=confirmed
    ?`current completed 5m ${passed}/5 confirmed; EMA + MACD mandatory checks PASS`
    :`current completed 5m is ${passed}/5${failed.length?` failed: ${failed.join(', ')}`:''}${mandatoryPassed?'':' | EMA/MACD mandatory confirmation missing'}`;
  return {status,score:`${passed}/5`,passed,mandatoryPassed,reason,checks,firstConfirmationIndex:confirmed?t:-1,firstConfirmationTime:confirmed?candles[t].closeTime:NaN};
}
...
function analyzeStep10(candles5,ind5,candles15,ind15,step8,step9,eventTime,currentPrice){
  ...
  if(step9.status!=='CONFIRMED')return {status:'WAIT',reason:'current completed 5m confirmation is not valid (need >=4/5 with EMA + MACD PASS)'};
  ...
  return {status:'READY',reason:`valid 15m setup + current 5m ${step9.score} (EMA+MACD mandatory) + current Fast Event trigger`,currentPrice:px,extensionAtr};
}
...
function finalizeEntryDecision(row,chart,finalSpread=NaN){
  ...
  if(s8.status==='PASS'){
    ...
    if(chart.step9?.status!=='CONFIRMED'){
      if(Number.isFinite(passed)&&passed>=4)coreCautions.push(`${chart.step9.score} but EMA/MACD mandatory confirmation missing`);
...
function logEntryDecisionDiagnostics(rows){
  ...
  window.CryptoFlowScanner=window.CryptoFlowScanner||{version:'V14.9',modules:{}};
  window.CryptoFlowScanner.version='V14.9';
