(() => {
  ...
  async function followFetchJson(url,{retries=3,timeout=15000,essential=false}={}){
    ...
    for(let attempt=0;attempt<=retries;attempt++){
      ...
      try{
        ...
        lastErr=e;if(e.name==='AbortError'&&scan?.controller?.signal.aborted)throw e;
  ...
  document.addEventListener('cryptooffer:scan-start',e=>stopAllFollowMonitors(e.detail?.reason||'New scan started'));
  ...
  window.CryptoFlowScanner.modules.follow={config:FOLLOW_CONFIG,liveConfig:FOLLOW_LIVE_CONFIG};
})();
